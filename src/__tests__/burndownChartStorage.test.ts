...
describe('BurndownChartStorage', () => {
  ...
  describe('loadCharts', () => {
    ...
    it('should load charts from existing file', async () => {
      mockReadFile.mockResolvedValue(JSON.stringify([mockChart]) as any);
    ...
    it('should convert date strings back to Date objects', async () => {
      const chartWithStringDates = {
        ...
        progressEntries: [
          { ...mockChart.progressEntries[0], date: mockChart.progressEntries[0].date.toISOString() }
        ]
      };
      mockReadFile.mockResolvedValue(JSON.stringify([chartWithStringDates]) as any);
  ...
  describe('updateChart', () => {
    ...
    it('should throw when chart not found', async () => {
      ...
      await expect(storage.updateChart(nonExistent))
        .rejects.toThrow('Chart with id missing-id not found');
    });
  });
  ...
  describe('deleteChart', () => {
    ...
    it('should throw when chart not found', async () => {
      ...
      await expect(storage.deleteChart('missing-id'))
        .rejects.toThrow('Chart with id missing-id not found');
    });
  });
  ...
  describe('getChartsByUser', () => {
    it('should return only charts belonging to the given user', async () => {
      const charts = [
        mockChart,
        { ...mockChart, id: 'chart-id-2', userId: 'user456' },
      ];
      mockReadFile.mockResolvedValue(JSON.stringify(charts) as any);
  ...
  describe('getActiveCharts', () => {
    it('should return only active charts', async () => {
      const charts = [
        mockChart,
        { ...mockChart, id: 'chart-id-2', isActive: false },
      ];
      mockReadFile.mockResolvedValue(JSON.stringify(charts) as any);
